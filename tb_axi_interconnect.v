...
module tb_axi_interconnect();
    ...
    generate
        ...
        for(g_idx = 0; g_idx < SLV_AMT; g_idx = g_idx + 1) begin : GEN_SLAVES
            axi_slave_if #(
                ...
                .s_AWBURST_i(s_AWBURST[TRANS_BURST_W*(g_idx+1)-1 -: TRANS_BURST_W]),
                .s_AWSIZE_i(s_AWSIZE[TRANS_DATA_SIZE_W*(g_idx+1)-1 -: TRANS_DATA_SIZE_W]),
                ...
                .s_ARBURST_i(s_ARBURST[TRANS_BURST_W*(g_idx+1)-1 -: TRANS_BURST_W]),
                .s_ARSIZE_i(s_ARSIZE[TRANS_DATA_SIZE_W*(g_idx+1)-1 -: TRANS_DATA_SIZE_W]),
    ...
    initial begin
        ...
        #50;
        master_write(0, 32'h8000_0000, 5'd0, 1, AxBURST_INCR, AxSIZE_4); 
        
        // 3. Đợi một khoảng thời gian, sau đó Master 1 đọc lại
        #500;
        master_read(1, 32'h8000_0000, 5'd0, 1, AxBURST_INCR, AxSIZE_4);
